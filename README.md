...
- To run this service in production, let's assume it will run as an Unbuntu service. For this we need a binary that starts a Kubernetes client from a particular Kubernetes config. In this binary we do not exit on error, we log the error and proceed.
